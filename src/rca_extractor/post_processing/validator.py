...
RANGE_RULES: list[RangeRule] = [
    ...
    RangeRule("factor_de_planta",                    0.05,    0.99, "—",
    ...
    RangeRule("consumo_de_agua_dulce_m3_mwh_1",      0,       20,  "m³/MWh",
...
def validate_ranges(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame([vars(r) for r in results]) if results else pd.DataFrame(
        columns=["archivo", "col", "valor", "estado", "mensaje"]
    )
...
def detect_outliers(df: pd.DataFrame, sigma: float = 3.0) -> pd.DataFrame:
    ...
    return pd.DataFrame(results) if results else pd.DataFrame(
        columns=["archivo", "col", "valor", "estado", "z_score", "mensaje"]
    )
