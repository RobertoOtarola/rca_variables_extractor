"""
dashboard/app.py — Dashboard Streamlit para RCA Extractor.

Arrancar:
    streamlit run dashboard/app.py

Requiere data/processed/results_lca.xlsx (Fase 4a).
Si no existe, busca results_normalized.xlsx → results.xlsx.
"""
...
from lca.calculator import calculate
...
def load_main() -> pd.DataFrame:
    for fname in ("results_lca.xlsx", "results_normalized.xlsx", "results.xlsx"):
        ...
        if p.exists():
            df = pd.read_excel(p)
            # Columna región normalizada
            df["region"] = (
                df["region_provincia_y_comuna"]
                .str.extract(r"Región\s+(?:de(?:l)?\s+)?([^,]+)")[0]
                .str.strip()
            )
            # Alias de región más frecuentes
            _aliases = {
...
def render_sidebar(df: pd.DataFrame):
    with st.sidebar:
        ...
        sel_mw   = st.slider("Potencia (MW)", mw_min, mw_max,
                              (mw_min, mw_max), step=0.5)
...
def apply_filters(df, sel_tech, sel_reg, sel_mw):
    mask = (
        df["potencia_nominal_bruta_mw"].between(sel_mw[0], sel_mw[1], inclusive="both")
    )
    if sel_tech != "Todas":
...
def render_overview(df: pd.DataFrame):
    ...
    with c1:
        ...
        fig = px.pie(tech_s, values="n", names="tech",
                     color="tech", color_discrete_map=TECH_COLORS,
                     hole=0.52, height=280)
        fig.update_traces(textinfo="percent+label", textposition="outside",
                          pull=[0.03] * len(tech_s))
    ...
    with c2:
        ...
        reg = (df.groupby("region")
                  .agg(potencia=("potencia_nominal_bruta_mw", "sum"),
                       n=("archivo", "count"))
                  .reset_index()
        ...
        fig.add_trace(go.Bar(
            x=reg["potencia"], y=reg["region"],
            orientation="h", name="MW",
            marker_color="#3B82F6",
    ...
    with c3:
        ...
        fig = px.histogram(vu, nbins=20, height=280,
                           color_discrete_sequence=["#10B981"])
        fig.update_layout(showlegend=False, bargap=0.05,
                          margin=dict(t=4, b=4, l=4, r=4),
                          xaxis_title="Años", yaxis_title="Proyectos")
        fig.add_vline(x=float(vu.median()), line_dash="dash",
                      line_color="#6B7280",
...
def render_land_power(df: pd.DataFrame):
    ...
    with c1:
        ...
        fig = px.histogram(plot_df, x="potencia_nominal_bruta_mw",
                           color="tech", color_discrete_map=TECH_COLORS,
                           nbins=45, log_y=True, height=270,
                           labels={"potencia_nominal_bruta_mw": "MW",
                                   "tech": "Tecnología"})
        fig.update_layout(bargap=0.04, margin=dict(t=4, b=4, l=4, r=4),
                          legend=dict(orientation="h", y=1.12))
        st.plotly_chart(fig, use_container_width=True)

    with c2:
        st.markdown("**Potencia vs Intensidad uso de suelo**")
        sc = df.dropna(subset=["potencia_nominal_bruta_mw",
                                "intensidad_de_uso_de_suelo_ha_mw_1"])
        fig = px.scatter(sc,
                         x="potencia_nominal_bruta_mw",
                         y="intensidad_de_uso_de_suelo_ha_mw_1",
                         color="tech", color_discrete_map=TECH_COLORS,
                         log_x=True, height=270,
                         hover_data=["archivo", "region"],
                         labels={"potencia_nominal_bruta_mw": "Potencia (MW)",
                                 "intensidad_de_uso_de_suelo_ha_mw_1": "ha/MW",
                                 "tech": ""})
        fig.update_traces(marker_size=5, marker_opacity=0.7)
        fig.update_layout(margin=dict(t=4, b=4, l=4, r=4),
                          legend=dict(orientation="h", y=1.12))
        st.plotly_chart(fig, use_container_width=True)
...
def render_lca(df: pd.DataFrame):
    ...
    st.caption("GEI estimados con factores IPCC AR6 · Agua de RCA cuando disponible, benchmark NREL si no · Tierra comparada con Ong et al. (2013)")
    ...
    with col1:
        ...
        ghg_df = (df.groupby("tech")["ghg_total_kt"]
                    .sum().reset_index()
                    .sort_values("ghg_total_kt", ascending=False))
        fig = px.bar(ghg_df, x="tech", y="ghg_total_kt",
                     color="tech", color_discrete_map=TECH_COLORS,
                     labels={"ghg_total_kt": "kt CO₂-eq", "tech": ""},
                     height=260, text_auto=".0f")
    ...
    with col2:
        ...
        e_df = (df.groupby("tech")["lifetime_energy_mwh"]
                  .sum().div(1e6).reset_index()
                  .rename(columns={"lifetime_energy_mwh": "TWh"})
                  .sort_values("TWh", ascending=False))
        fig = px.bar(e_df, x="tech", y="TWh",
                     color="tech", color_discrete_map=TECH_COLORS,
                     labels={"tech": ""}, height=260, text_auto=".0f")
        fig.update_layout(showlegend=False,
                          margin=dict(t=4, b=4, l=4, r=4))
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col3:
        ...
        if w_df.empty:
            ...
            fig = px.box(w_df, x="tech", y="water_intensity_m3_mwh",
                         color="tech", color_discrete_map=TECH_COLORS,
...
def render_map(geo_df: pd.DataFrame):
    ...
    map_df["size_px"] = (map_df["potencia_nominal_bruta_mw"].clip(upper=300)
                          .fillna(10))
    ...
    fig = px.scatter_mapbox(
        ...
        hover_data={
            ...
            "coord_method": True,
            "lat": False, "lon": False, "size_px": False,
        },
        zoom=4,
        center={"lat": -30, "lon": -70},
        mapbox_style="carto-positron",
        labels={"tech": "Tecnología",
...
def main():
    ...
    if df.empty:
        st.error(
            "No se encontró `data/processed/results*.xlsx`.\n\n"
            "Ejecuta primero:\n"
            "```bash\n"
            "python main.py --pdf-folder data/raw --output data/processed/results.xlsx\n"
            "python -m post_processing.run\n"
            "python -m lca.run\n"
            "```"
        )
        return
