...
_NON_WGS84_RE = re.compile(
    r"PSAD|Provisorio\s+Sudamericano|SAD\s*56|SAD\s*69|Chile\s*\d{4}", re.I
)
...
PATTERNS = [
    ...
    ("E_colon_N",  True,
     re.compile(r"\bE[:\s_]+" + _N + _MS + _SEP + _NR + r"[:\s_]+" + _N, re.I)),
    ("N_colon_E",  False,
     re.compile(_NR + r"[:\s_]+" + _N + _MS + _SEP + r"E[:\s_]+" + _N, re.I)),
